...
int main(int argc, char *argv[]) {
    ...
    if (db){
        ...
        ////////////////////////////////////////////////////////////////////
        //variables for checking if courses repeat with different grades
        int check[1000] = {0};
        //find if repeat && different
        for (int i = 0; i < size; i++)
        {
          for (int j = 0; j < students[i].load; j++)
          {
            for (int k = 0; k < students[i].load; k++)
            {
              if (j != k)
              {
                if ((strcmp(students[i].courses[j], students[i].courses[k]) == 0) && (students[i].grades[j] != students[i].grades[k]))
                {
