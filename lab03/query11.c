...
int main(int argc, char *argv[]) {
    ...
    if (db){
        ...
        for (int i = 0; i < size; ++i){ // process all the student records in database
            ...
            ////////////////////////////////////////////////////////////////////
            int checkPhilos = 0;
            char philosStr[11] = "Philosophy";
            //check if student has the required courses and grades
            for (int i = 0; i < s.load; ++i)
            {
                if ((strcmp(s.courses[i], philosStr) == 0) || (s.grades[i] == 10))
                {
                  checkPhilos = 1;
                }
            }
            ////////////////////////////////////////////////////////////////////
            if(checkPhilos){ // *** first filter, conditions on the student
