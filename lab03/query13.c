...
int main(int argc, char *argv[]) {
    ...
    if (db){
        ...
        ////////////////////////////////////////////////////////////////////
        //variable for avrg check
        double gradeSum;
        double avrgGrade;
        int tmp;
        int check[1000] = {0};
        //find everyones actal average
        for (int i = 0; i < size; ++i)
        {
          avrgGrade = 0;
          gradeSum = 0;
          ...
          double studentLoad = students[i].load;
          avrgGrade = gradeSum / studentLoad;
          //Check if actual avrg is same as supposed one
          if (students[i].average == avrgGrade)
          {
            check[i] = 1;
          }
          if (students[i].load == 0)
          {
            check[i] = 1;
          }
        ...
        for (int i = 0; i < size; ++i){ // process all the student records in database
            ...
            if(check[i] == 0){ // *** first filter, conditions on the student
